(function($){
	$.fn.simplePagination = function(perPage, opened, onClass, paginationSelector){
		...
		var to_append = '<ul><li class="prev" style="display:none"><a href="#"><span class="spnPageNumber">prev</span></a></li>';
		for (i = 1; i <= no_pages; i++) {
			to_append += '<li class="pagination-page-numbers';
			if (i === opened) {
				to_append += ' '+onClass;
			}
			to_append += '"><a href="#" class="PageNumbers"><span class="spnPageNumber">' + i + '</span></a></li>';
		}
		to_append += '<li class="next" style="display:none"><a href="#" class="PageNumbers"><span class="spnPageNumber">next</span></a></li></ul>';
		if (opened !== 1) {
			if (opened === no_pages) {
			    pagination.find('.next').find('span').unwrap().wrap('<span class="spnPageNumber"></span>');
			}
		...
		if (pagination.find('.'+onClass).index() == 1) {
		    pagination.find('.prev').find('span').unwrap().wrap('<span class="spnPageNumber"></span>');
		}
		...
		function nextClick() {
			if (!pagination.find('.'+onClass).next().hasClass('next')) {
				...
				if (pagination.find('.'+onClass).index() == 2) {
				    pagination.find('.prev').find('span:last').unwrap().wrap('<a href="#" class="PageNumbers"></a>').parent().bind("click", prevClick);
				}
				if (pagination.find('.'+onClass).index() == no_pages) {
				    $(this).find('span').unwrap().wrap('<span class="spnPageNumber"></span>');
				}
			...
			if (!pagination.find('.'+onClass).prev().hasClass('prev')) {
				...
				if (pagination.find('.'+onClass).index() < no_pages) {
					if (pagination.find('.next').find('a').length == 0) {
					    pagination.find('.next').find('span:last').unwrap().wrap('<a href="#" class="PageNumbers"></a>').parent().bind('click', nextClick);
					}
				}
				if (pagination.find('.'+onClass).index() === 1) {
				    pagination.find('.prev').find('span').unwrap().wrap('<span class="spnPageNumber"></span>');
